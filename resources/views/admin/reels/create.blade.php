...
@section('content')
    <div class="row mb-5">
        <div class="col-md-8">
            <h2 class="welcome-title mb-1">{{ __('New Reel') }}</h2>
            <p class="text-muted mb-0">{{ __('Create a new story-style reel for the website.') }}</p>
        </div>
                    ...
                    <div class="panel-body pt-0">
                            ...
                            @foreach($locales as $locale)
                                ...
                                <div class="tab-pane fade {{ $locale['code'] === $firstSelectedLocaleCode ? 'show active' : '' }} {{ $isSelected ? '' : 'd-none' }}"
                                    ...
                                    @include('admin.partials.ai-locale-toolbar', [
                                        ...
                                        'translateUrl' => route('admin.reels.ai.translate-draft'),
                                        'seoUrl'       => '',
                                        'draftMode'    => true,
                                    ])
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <textarea name="descriptions[{{ $locale['code'] }}]"
                                                  ...
                                                  {{ $isSelected ? '' : 'disabled' }}>{{ old('descriptions.'.$locale['code']) }}</textarea>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary px-5 py-3 rounded-3 shadow-premium d-flex align-items-center gap-2">
                                ...
                                <span class="fw-bold">{{ __('Save Reel') }}</span>
                            </button>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="panel-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category" class="form-select @error('category') is-invalid @enderror">
                                    <option value="new"     @selected(old('category', 'new') === 'new')>🆕 {{ __('New') }}</option>
                                    <option value="sale"    @selected(old('category') === 'sale')>🔥 {{ __('Sale') }}</option>
                                    <option value="project" @selected(old('category') === 'project')>🏗️ {{ __('Project') }}</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="video_url" id="video_url_input"
                                       ...
                                       value="{{ old('video_url') }}" placeholder="{{ __('Stored path or URL') }}">
...
<script>
// Override buildDraftTranslations for reels (uses titles[] not names[])
document.addEventListener('DOMContentLoaded', function () {
    const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';

    document.querySelectorAll('.ai-locale-tools').forEach((tools) => {
        // Hide the SEO button since reels have no SEO fields
        ...
        // Replace the generic click handler with a reel-specific one
        ...
        newBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (draftMode && form instanceof HTMLFormElement) {
                    ...
                    fd.forEach((val, key) => {
                        const m = key.match(/^titles\[(.+)\]$/);
                        if (m) { titles[m[1]] = val; }
                        const d = key.match(/^descriptions\[(.+)\]$/);
                        if (d) { descriptions[d[1]] = val; }
                        const s = key.match(/^slugs\[(.+)\]$/);
                        if (s) { slugs[s[1]] = val; }
                    });
                    const locales = Object.keys({ ...titles, ...descriptions });
                    body.translations = locales.reduce((acc, lc) => {
                        acc[lc] = { title: titles[lc] || '', description: descriptions[lc] || '' };
                        return acc;
                    }, {});
                }
