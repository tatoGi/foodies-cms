...
@section('content')
    <div class="row mb-5">
        <div class="col-md-8">
            <h2 class="welcome-title mb-1">{{ __('New Product') }}</h2>
            <p class="text-muted mb-0">{{ __('Create a new product with blocks and SEO.') }}</p>
        </div>
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" id="pageForm" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="panel-body pt-0">
                            ...
                            <div class="panel-body">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="cover_image" id="cover_image_input" class="form-control mb-2 @error('cover_image') is-invalid @enderror" value="{{ old('cover_image') }}" placeholder="{{ __('Stored path') }}">
                                ...
                                <div class="d-flex flex-wrap gap-3 mt-3">
                                    ...
                                    <div class="form-check form-switch p-0 ps-5 mb-0">
                                        <input class="form-check-input ms-n5" type="checkbox" id="published" name="published" value="1" @checked(old('published'))>
                                ...
                                <div class="tab-pane fade {{ $locale['code'] === $firstSelectedLocaleCode ? 'show active' : '' }} {{ $isSelected ? '' : 'd-none' }}"
                                    ...
                                    <div class="mb-4">
                                        @include('admin.partials.ai-locale-toolbar', [
                                            ...
                                            'translateUrl' => route('admin.products.ai.translate-draft'),
                                            'seoUrl' => route('admin.products.ai.seo-draft'),
                                            'draftMode' => true,
                                        ])
                                        <label class="form-label small fw-bold text-muted uppercase letter-spacing-1">
                                            {{ __('Product Blocks') }}
                                                    ...
                                                    <div class="card page-block-card" data-block-key="{{ $block['key'] }}" data-instance-key="{{ $block['instance_key'] }}" data-locale-code="{{ $locale['code'] }}">
                                                        <div class="card-header bg-light-soft d-flex align-items-center justify-content-between py-2">
                                                            ...
                                                            <div class="btn-group btn-group-sm">
                                                                <button type="button" class="btn btn-light move-block-up"><i class="bi bi-arrow-up"></i></button>
                                                                <button type="button" class="btn btn-light move-block-down"><i class="bi bi-arrow-down"></i></button>
                                                                <button type="button" class="btn btn-light remove-block-instance" title="{{ __('Remove') }}"><i class="bi bi-trash"></i></button>
                                                                <button type="button" class="btn btn-light toggle-block-body"><i class="bi bi-chevron-up"></i></button>
                                                            </div>
                                                        </div>
                                                            ...
                                                            @foreach($editor['fields'] as $field)
                                                                ...
                                                                <div class="mb-3">
                                                                    ...
                                                                    @elseif($fieldType === 'gallery')
                                                                        <textarea name="blocks[{{ $locale['code'] }}][{{ $block['key'] }}][data][{{ $fieldKey }}]" class="form-control form-control-sm mb-2" rows="3">{{ is_array($block['data'][$fieldKey] ?? null) ? implode(PHP_EOL, (array)$block['data'][$fieldKey]) : $fieldValue }}</textarea>
                                            ...
                                            <div class="tab-pane fade {{ $locale['code'] === $firstSelectedLocaleCode ? 'show active' : '' }} {{ $isSelected ? '' : 'd-none' }}"
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <div class="keywords-tag-input" data-keywords-widget>
                                                        ...
                                                        <input type="hidden" class="keywords-tag-hidden" name="keywords[{{ $locale['code'] }}]" value="{{ old('keywords.'.$locale['code']) }}" {{ $isSelected ? '' : 'disabled' }}>
                                                    </div>
                                                </div>
                                                <div class="mb-3">
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary px-5 py-3 rounded-3 shadow-premium d-flex align-items-center gap-2">
                                ...
                                <span class="fw-bold">{{ __('Save Product') }}</span>
                            </button>
                        </div>
                    </div>
                </div>
