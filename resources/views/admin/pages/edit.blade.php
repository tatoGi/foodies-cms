...
<style>
    ...
    .linked-content-panel .panel-body {
        flex: 0 0 auto;
    }
    .tox-tinymce {
...
@section('content')
    <div class="row mb-5">
        <div class="col-md-8">
            <h2 class="welcome-title mb-1">{{ __('Edit Page') }}</h2>
            <p class="text-muted mb-0">{{ __('Update page data and selected template.') }}</p>
        </div>
        <div class="col-md-4 text-md-end mt-3 mt-md-0">
            <a href="{{ route('admin.pages.index') }}" class="btn btn-light-soft px-4 py-2 rounded-3 d-inline-flex align-items-center gap-2">
                    ...
                    <div class="panel-body pt-0">
                        <div class="tab-content tab-content-premium" id="langTabsContent">
                            @foreach($locales as $locale)
                                ...
                                <div class="tab-pane fade {{ $locale['code'] === $firstSelectedLocaleCode ? 'show active' : '' }} {{ $isSelected ? '' : 'd-none' }}"
                                    ...
                                    <div class="mb-4">
                                        <label class="form-label small fw-bold text-muted uppercase letter-spacing-1">
                                            {{ __('Description / Excerpt') }}
                                        </label>
                                        <textarea name="descriptions[{{ $locale['code'] }}]" 
                                                  class="form-control editor-field @error('descriptions.'.$locale['code']) is-invalid @enderror"
                                                  rows="5" {{ $isSelected ? '' : 'disabled' }}>{{ old('descriptions.'.$locale['code'], $translations[$locale['code']]->description ?? '') }}</textarea>
                                    ...
                                    <div class="mb-4">
                                        @include('admin.partials.ai-locale-toolbar', [
                                            ...
                                            'translateUrl' => route('admin.pages.ai.translate', $page),
                                            'seoUrl' => route('admin.pages.ai.seo', $page),
                                        ])
                                        <label class="form-label small fw-bold text-muted uppercase letter-spacing-1">
                                            {{ __('Page Blocks') }}
                                        </label>
                                            ...
                                            @foreach($localeBlocks[$locale['code']] ?? [] as $block)
                                                ...
                                                @if($editor)
                                                    <div class="card page-block-card" data-block-key="{{ $block['key'] }}" data-instance-key="{{ $block['instance_key'] }}" data-locale-code="{{ $locale['code'] }}">
                                                        ...
                                                        <div class="card-body block-body-content d-none">
                                                            ...
                                                            @foreach($editor['fields'] as $field)
                                                                ...
                                                                <div class="mb-3">
                                                                    ...
                                                                    @elseif($fieldType === 'video')
                                                                        ...
                                                                        @if($fieldValue !== '')
                                                                            @php
                                                                                $videoUrl = \Illuminate\Support\Str::startsWith($fieldValue, ['http://', 'https://', '/'])
                                                                                    ? $fieldValue
                                                                                    : asset('storage/'.$fieldValue);
                                                                            @endphp
                                                                            <div class="mb-2 image-preview-wrap position-relative d-inline-block">
                                                                                <a href="{{ $videoUrl }}" target="_blank" class="small text-decoration-none d-inline-block mb-2">{{ __('Current video') }}</a>
                                                                                <div>
                                                                                    <video src="{{ $videoUrl }}" muted playsinline preload="metadata" style="max-width: 240px; max-height: 140px; border-radius: 8px; border: 1px solid #dee2e6; background: #000;"></video>
                                                                                </div>
                                                                                <button type="button"
                                                                                        class="btn btn-sm btn-danger position-absolute remove-single-image"
                                                                                        data-confirm="{{ __('Remove video?') }}"
                                                                                        style="top: -6px; right: -6px; line-height: 1; padding: 2px 7px;">&times;</button>
                                                                            </div>
                                                                        @endif
                                                                        <input type="text"
                                                                               ...
                                                                               placeholder="{{ __('Stored path') }}"
                                                                               value="{{ $fieldValue }}">
                                                                        <button type="button" class="btn btn-outline-secondary btn-sm open-media-picker mb-2" data-picker-mode="video">
                                                                            {{ __('Choose from Media') }}
                                                                        </button>
                                                                        <input type="file"
                                                                               name="blocks[{{ $locale['code'] }}][{{ $block['key'] }}][files][{{ $fieldKey }}]"
                                                                    ...
                                                                    @elseif($fieldType === 'gallery')
                                                                        <textarea id="gallery-{{ $locale['code'] }}-{{ $block['key'] }}-{{ $fieldKey }}" name="blocks[{{ $locale['code'] }}][{{ $block['key'] }}][data][{{ $fieldKey }}]" class="form-control form-control-sm mb-2" rows="3" placeholder="{{ __('Image paths separated by comma or new line') }}">{{ is_array($block['data'][$fieldKey] ?? null) ? implode("\n", (array) $block['data'][$fieldKey]) : $fieldValue }}</textarea>
                                        ...
                                        @foreach($locales as $locale)
                                            ...
                                            <div class="tab-pane fade {{ $locale['code'] === $firstSelectedLocaleCode ? 'show active' : '' }} {{ $isSelected ? '' : 'd-none' }}"
                                                ...
                                                @php
                                                    ...
                                                    $seoUrl = isset($page) ? route('admin.pages.ai.seo', $page) : route('admin.pages.ai.seo-draft');
                                                    $draftMode = isset($page) ? 'false' : 'true';
                                                @endphp
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <div class="keywords-tag-input" data-keywords-widget>
                                                        ...
                                                        <input type="hidden"
                                                               ...
                                                               name="meta_keywords[{{ $locale['code'] }}]"
                                                               value="{{ old('meta_keywords.'.$locale['code'], $translations[$locale['code']]->keywords ?? '') }}"
                                                               {{ $isSelected ? '' : 'disabled' }}>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary px-5 py-3 rounded-3 shadow-premium d-flex align-items-center gap-2">
                                ...
                                <span class="fw-bold">{{ __('Update Page') }}</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="admin-form-sidebar">
                    ...
                    <div class="dashboard-panel premium-shadow mb-4 settings-panel">
                        ...
                        <div class="panel-body">
                            <div class="mb-3">
                                <label class="form-label fw-bold small text-muted uppercase letter-spacing-1">{{ __('Template') }}</label>
                                <select name="template" class="form-select @error('template') is-invalid @enderror" required>
                                    <option value="">{{ __('Select Template') }}</option>
                                    @foreach($templates as $template)
                                        <option value="{{ $template['slug'] }}" @selected(old('template', $page->template) === $template['slug'])>
                                            {{ $template['name'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold small text-muted uppercase letter-spacing-1">{{ __('Parent Page') }}</label>
                                <select name="parent_id" class="form-select @error('parent_id') is-invalid @enderror">
                                    <option value="">{{ __('No Parent') }}</option>
                                    @foreach($parentPages as $parentPage)
                                        <option value="{{ $parentPage['id'] }}" @selected((int) old('parent_id', (int) $page->parent_id) === $parentPage['id'])>
                                            {{ $parentPage['title'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                <label class="form-label fw-bold small text-muted uppercase letter-spacing-1">{{ __('Sort Order') }}</label>
                                <input type="number" name="sort_order" min="0" class="form-control @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', $page->sort_order) }}">
                                @error('sort_order')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>

                            <div class="mb-3" data-media-field-wrapper>
                                <label class="form-label fw-bold small text-muted uppercase letter-spacing-1">{{ __('Feature Image Path') }}</label>
                                @php
                                    $featureImage = (string) old('feature_image', $page->feature_image);
                                    $featureImageUrl = $featureImage !== '' && !str_starts_with($featureImage, 'http://') && !str_starts_with($featureImage, 'https://') && !str_starts_with($featureImage, '/')
                                        ? asset('storage/'.$featureImage)
                                        : $featureImage;
                                @endphp
                                @if($featureImage !== '')
                                    <div class="mb-2">
                                        <a href="{{ $featureImageUrl }}" target="_blank" class="small text-decoration-none">{{ __('Current image') }}</a>
                                    </div>
                                @endif
                                <input type="text" name="feature_image" class="form-control @error('feature_image') is-invalid @enderror mb-2" value="{{ $featureImage }}">
                                <button type="button" class="btn btn-outline-secondary btn-sm open-media-picker" data-picker-mode="image" data-source-context="pages/feature_image">
                                    {{ __('Choose from Media') }}
                                </button>
                                @error('feature_image')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                            ...
                            <div class="form-check form-switch p-0 ps-5 mt-4">
                                <input class="form-check-input ms-n5" type="checkbox" id="published" name="published" value="1" @checked(old('published', $page->published))>
                                <label class="form-check-label fw-bold" for="published">{{ __('Published') }}</label>
                            </div>

                            <div class="form-check form-switch p-0 ps-5 mt-3">
                                <input class="form-check-input ms-n5" type="checkbox" id="is_home" name="is_home" value="1" @checked(old('is_home', $page->is_home))>
                                <label class="form-check-label fw-bold" for="is_home">{{ __('Homepage') }}</label>
                                <div class="form-text">{{ __('Use this page as the main homepage and root URL target.') }}</div>
                            </div>
                        </div>
                    </div>

                    <div class="dashboard-panel premium-shadow mb-4 linked-content-panel">
                        <div class="panel-header border-bottom-0">
                            <div class="panel-header-title">
                                <i class="bi bi-grid-3x3-gap me-2 text-primary"></i>
                                <span>{{ __('Page Blocks') }}</span>
                            </div>
                        </div>
                        <div class="panel-body">
                            @if(count($blockTypes) === 0)
                                <div class="text-muted small">{{ __('No page blocks available.') }}</div>
                            @else
                    ...
                    <div class="dashboard-panel premium-shadow mb-4 linked-content-panel">
                        <div class="panel-header border-bottom-0 d-flex justify-content-between align-items-center">
                            <div class="panel-header-title">
                                <i class="bi bi-journal-text me-2 text-primary"></i>
                                <span>{{ __('Linked Posts') }}</span>
                            </div>
                            <button type="button" class="btn btn-sm btn-light-soft" id="select-all-posts">{{ __('Select All') }}</button>
                        </div>
                        <div class="panel-body">
                            <select
                                id="linked-posts-select"
                                name="post_ids[]"
                                class="form-select @error('post_ids') is-invalid @enderror"
                                multiple
                                data-placeholder="{{ __('Search and select posts...') }}"
                            >
                                @foreach($availablePosts as $postItem)
                                    <option value="{{ $postItem['id'] }}" @selected(in_array((int) $postItem['id'], $selectedPostIds, true))>
                                        {{ $postItem['title'] }}{{ $postItem['slug'] !== '' ? ' (/'.$postItem['slug'].')' : '' }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="form-text">{{ __('Type to search posts and assign them to this page.') }}</div>
                            @error('post_ids')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                            @error('post_ids.*')
                    ...
                    <div class="dashboard-panel premium-shadow mb-4 linked-content-panel">
                        <div class="panel-header border-bottom-0 d-flex justify-content-between align-items-center">
                            <div class="panel-header-title">
                                ...
                                <span>{{ __('Linked Products') }}</span>
                            </div>
                            <button type="button" class="btn btn-sm btn-light-soft" id="select-all-products">{{ __('Select All') }}</button>
                        </div>
                        <div class="panel-body">
                            <select
                                id="linked-products-select"
                                name="product_ids[]"
                                class="form-select @error('product_ids') is-invalid @enderror"
                                multiple
                                data-placeholder="{{ __('Search and select products...') }}"
                            >
                                @foreach($availableProducts as $productItem)
                                    <option value="{{ $productItem['id'] }}" @selected(in_array((int) $productItem['id'], $selectedProductIds, true))>
                                        {{ $productItem['title'] }}{{ $productItem['slug'] !== '' ? ' (/'.$productItem['slug'].')' : '' }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="form-text">{{ __('Type to search products and assign them to this page.') }}</div>
                            @error('product_ids')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                            @error('product_ids.*')
...
document.addEventListener('DOMContentLoaded', function() {
    const linkedPostsSelect = document.getElementById('linked-posts-select');
    let tsPostsInstance = null;
    if (linkedPostsSelect) {
        tsPostsInstance = new TomSelect(linkedPostsSelect, {
            ...
            render: {
                option: function(data, escape) {
                    return '<div class="linked-item-option">' +
                        '<div class="linked-item-icon"><i class="bi bi-journal-text"></i></div>' +
                        '<div>' +
                ...
                item: function(data, escape) {
                    return '<div><i class="bi bi-journal-text me-2"></i>' + escape(data.text) + '</div>';
                }
            }
        });
    }

    const linkedProductsSelect = document.getElementById('linked-products-select');
    let tsProductsInstance = null;
    if (linkedProductsSelect) {
        tsProductsInstance = new TomSelect(linkedProductsSelect, {
            plugins: ['remove_button'],
            maxOptions: 5000,
            hideSelected: true,
            closeAfterSelect: false,
            ...
            render: {
                option: function(data, escape) {
                    return '<div class="linked-item-option">' +
                        '<div class="linked-item-icon"><i class="bi bi-box-seam-fill"></i></div>' +
                        '<div>' +
                            '<div class="fw-bold">' + escape(data.text) + '</div>' +
                            '<div class="small text-muted">' + (data.value || '') + '</div>' +
                        '</div>' +
                ...
                item: function(data, escape) {
                    return '<div><i class="bi bi-box-seam-fill me-2"></i>' + escape(data.text) + '</div>';
                }
            }
        });
    }

    const makeSelectAllToggle = (btn, getInstance) => {
        if (!btn) return;
        btn.addEventListener('click', function () {
            const ts = getInstance();
            if (!ts) return;
            const allKeys = Object.keys(ts.options);
            const allSelected = allKeys.every(k => ts.items.includes(k));
            if (allSelected) {
                ts.clear();
                btn.textContent = '{{ __("Select All") }}';
            } else {
                ts.setValue(allKeys);
                btn.textContent = '{{ __("Deselect All") }}';
            }
        });
    };

    makeSelectAllToggle(document.getElementById('select-all-posts'), () => tsPostsInstance);
    makeSelectAllToggle(document.getElementById('select-all-products'), () => tsProductsInstance);
...
@include('admin.partials.block-instance-script')
    ...
    document.querySelectorAll('[id^="name-"]').forEach(function(nameInput) {
        const localeCode = nameInput.id.replace('name-', '');
